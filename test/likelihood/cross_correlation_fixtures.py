...
from cryolike.stack.template import _fourier_circles
...
class parameters():
    ...
    @property
    def float_type(self):
        return get_float_dtype(self.precision)

    @property
    def complex_type(self):
        return get_complex_dtype(self.precision)
    ...
    def duplicate(self, *,
        ...
        PrecisionLevel: PrecisionLevel | None = None,
        ...
        return parameters(
            ...
            precision=PrecisionLevel if PrecisionLevel is not None else self.precision,
            wavevector=wv.to(self.float_type),
    ...
    def default():
        ...
        default_float = get_float_dtype(default_precision)
        return parameters(
            device='cpu',
            n_pixels=128,
            ...
            abs_tolerance_cross_correlation=1e-5,
            rel_tolerance_cross_correlation=1e-3,
            abs_tolerance_log_likelihood=1e-3,
            rel_tolerance_log_likelihood=1e-3,
        )
    
    def __repr__(self) -> str:
        return (f"parameters(device='{self.device}', n_pixels={self.n_pixels}, "
                f"precision={self.precision}, wavevector={self.wavevector}, "
                f"max_displacement={self.max_displacement}, "
                f"abs_tolerance_cross_correlation={self.abs_tolerance_cross_correlation}, "
                f"rel_tolerance_cross_correlation={self.rel_tolerance_cross_correlation}, "
                f"abs_tolerance_log_likelihood={self.abs_tolerance_log_likelihood}, "
                f"rel_tolerance_log_likelihood={self.rel_tolerance_log_likelihood})")
...
def make_cases() -> list[parameters]:
    ...
    # with_double_precision = [x.duplicate(
    #     PrecisionLevel=PrecisionLevel.SINGLE, 
    #     abs_tolerance_cross_correlation=1e-6, rel_tolerance_cross_correlation=1e-6, 
...
def make_polar_grid(n_pixels: int) -> UniformPolarGrid:
...
def make_planewave_templates(
    ...
    fourier_circles = _fourier_circles(
        polar_grid.theta_shell,
        viewing_angles.polars,
        viewing_angles.azimus,
        viewing_angles.gammas,
    ) # (n_viewings, n_inplanes, xyz)
    fourier_slices = fourier_circles[:,None,:,:] * polar_grid.radius_shells[None,:,None,None] # (n_viewings, n_shells, n_inplanes, xyz)
    return torch.exp(2 * pi * 1j * torch.matmul(fourier_slices, wavevector_planewave))
...
def get_planar_ctf(grid: UniformPolarGrid, phi_S: float) -> torch.Tensor:
    return (2.0 * grid.radius_shells)[:,None] * torch.cos(grid.theta_shell - phi_S)[None,:]
...
def planewave_planar_planewave_planar(
    ...
    return result # (n_image, n_templates, n_displacements, n_inplanes)