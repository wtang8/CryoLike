...
class LogLikelihoodPlanarCTFPlanewaves():
    ...
    def __init__(
        ...
        self.device = device
        angle_planar_ctf_template_tensor = torch.tensor(angle_planar_ctf_template, dtype=self.float_type, device=device)
        angle_planar_ctf_image_tensor = torch.tensor(angle_planar_ctf_image, dtype=self.float_type, device=device)
        ...
        self.I_T_T_form = planewave_planar_planewave_planar(
            wavevector_planewave_templates,
            wavevector_planewave_templates,
            torch.tensor([0.0], dtype=self.float_type, device=device),
            torch.tensor([0.0, 0.0], dtype=self.float_type, device=device),
